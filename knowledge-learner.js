...
// Module for reading historical conversations and building knowledge base with multi-workspace support
...
async function connectToMongoDB(mongoURI) {
  ...
  try {
    ...
    await learnedQACollection.createIndex({ workspaceId: 1 });
 ...
 */
async function learnFromChannelHistory(client, workspaceId = 'default') {
    ...
    try {
      ...
      if (existing) {
        ...
        await learnedQACollection.insertOne({
          ...
          programName: group.programName,
          channelName: channelName,
          workspaceId: workspaceId,  // Add workspace ID
          confidence: confidence,
          useCount: 1,
          createdAt: new Date(),
          lastUpdated: new Date()
...
/**
 * Learn from the bot's own Q&A history for a specific workspace
 * @param {string} workspaceId - Workspace ID
 * @returns {Promise<number>} - Number of Q&A pairs learned
 */
async function learnFromBotHistory(workspaceId = 'default') {
  ...
  try {
    console.log(`Learning from bot history for workspace ${workspaceId}...`);
    let learnedCount = 0;
    
    // Get questions that received matched responses for this workspace
    ...
    for (const qa of matchedQuestions) {
      ...
      try {
        ...
        if (existing) {
          ...
          // Also add to cache
          const cacheKey = `${workspaceId}:${qa.programName || 'General'}:${qa.question.toLowerCase().substring(0, 30)}`;
          learnedQACache.set(cacheKey, {
            answer: qa.response,
            confidence: 0.95,
            programName: qa.programName || 'General'
...
async function findLearnedAnswer(question, programName, workspaceId = 'default') {
  ...
  if (isConnected && learnedQACollection) {
    try {
      // Search for similar questions in this program and workspace
 ...
 * @param {string} workspaceId - Workspace ID
 */
function schedulePeriodicLearning(client, workspaceId = 'default') {
  ...
  setTimeout(async () => {
    ...
    await learnFromChannelHistory(client, workspaceId);
    await learnFromBotHistory(workspaceId);
...
/**
 * Debug function to search the knowledge base
 * @param {string} searchTerm - Search term
 * @param {string} workspaceId - Workspace ID (optional)
 * @returns {Promise<Array>} - Array of matching Q&A pairs
 */
async function debugSearch(searchTerm, workspaceId = null) {
  if (!isConnected || !learnedQACollection) {
    console.log('Cannot debug search: MongoDB not connected');
    return [];
  }
